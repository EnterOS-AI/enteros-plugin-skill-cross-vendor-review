#!/usr/bin/env python3
"""
Smoke tests for molecule-skill-cross-vendor-review.
...
class TestPluginManifest(unittest.TestCase):
    ...
    def test_name(self):
        self.assertEqual(self.manifest['name'], 'molecule-skill-cross-vendor-review')
    ...
    def test_skill_declared(self):
        self.assertIn('cross-vendor-review', self.manifest.get('skills', []))


class TestCrossVendorReviewSkill(unittest.TestCase):
    """Verify skills/cross-vendor-review/SKILL.md is well-formed."""

    SKILL_PATH = os.path.join(REPO_ROOT, 'skills', 'cross-vendor-review', 'SKILL.md')
    ...
    def test_frontmatter_name(self):
        ...
        _, frontmatter, _ = parts
        data = yaml.safe_load(frontmatter)
        self.assertEqual(data['name'], 'cross-vendor-review')
    ...
    def test_body_documents_vendor_models(self):
        with open(self.SKILL_PATH) as f:
            content = f.read()
        # Should mention at least one non-Claude vendor
        self.assertTrue(
            any(v in content for v in ['GPT', 'Gemini', 'Codex']),
            "SKILL.md should reference non-Claude vendors"
        )
...
class TestValidatePlugin(unittest.TestCase):
    ...
    def test_validate_plugin_exits_zero(self):
        ...
        self.assertIn('molecule-skill-cross-vendor-review', result.stdout)
